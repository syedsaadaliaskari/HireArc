      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-linear-to-br from-indigo-100 to-purple-100 flex items-center justify-center text-indigo-600 font-bold text-xl shrink-0">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">{applicant.userId.email}</p>
          ...
          {/* Skills */}
          {applicant.userId.skills && applicant.userId.skills.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">
                Skills
              </p>
              <div className="flex flex-wrap gap-2">
                {applicant.userId.skills.map((skill) => (
                  <span
                    key={skill}
                    className="text-xs bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full ring-1 ring-indigo-200 font-medium"
                  >
                    {skill}
                  </span>
                ))}
          ...
          {applicant.coverLetter ? (
            <div>
              ...
              <div className="bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-600 leading-relaxed whitespace-pre-line">
                {applicant.coverLetter}
              </div>
            </div>
          ) : (
          ...
          <div>
            ...
            {applicant.cvPath ? (
              <a
                ...
                className="inline-flex items-center gap-2 bg-indigo-600 text-white text-sm font-semibold px-4 py-2.5 rounded-lg hover:bg-indigo-700 active:scale-95 transition-all"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
          ...
          {applicant.status === "PENDING" && (
            <div className="flex gap-3 pt-2 border-t border-gray-100">
              <button
                ...
                onClick={() => {
                  onUpdateStatus(applicant._id, "ACCEPTED");
                  onClose();
                }}
                className="flex-1 bg-emerald-600 text-white text-sm font-semibold py-2.5 rounded-lg hover:bg-emerald-700 active:scale-95 transition-all disabled:opacity-50"
              >
                ✓ Accept
              </button>
              <button
                disabled={updatingId === applicant._id}
                onClick={() => {
                  onUpdateStatus(applicant._id, "REJECTED");
                  onClose();
                }}
                className="flex-1 bg-red-500 text-white text-sm font-semibold py-2.5 rounded-lg hover:bg-red-600 active:scale-95 transition-all disabled:opacity-50"
              >
                ✕ Reject
              </button>
            </div>
...
export default function DashboardPage() {
    ...
    const fetchData = async () => {
      try {
        if (user.role === "EMPLOYER") {
          ...
          const counts = await Promise.all(
            jobsData.map(async (job) => {
              try {
                ...
                const data: Applicant[] = await r.json();
                return { jobId: job._id, applicants: data };
              } catch {
          ...
          const applicantsMap: Record<string, Applicant[]> = {};
          let total = 0;
          counts.forEach(({ jobId, applicants }) => {
  ...
  // Find jobId for modal update
  const handleModalUpdate = (
    applicationId: string,
    status: "ACCEPTED" | "REJECTED",
  ) => {
    const jobId = Object.keys(applicants).find((jId) =>
      applicants[jId].some((a) => a._id === applicationId),
    );
    if (jobId) updateStatus(applicationId, jobId, status);
  };
                  ...
                  <div key={job._id}>
                    <div className="px-6 py-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                      <div className="flex items-start gap-3">
                        <div className="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-600 font-bold text-base border border-indigo-100 shrink-0">
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        >
                          {applicantsLoading === job._id ? (
                            ...
                            <>
                              {applicants[job._id]?.length ?? 0} Applicants
                              <svg
                                    ...
                                    {applicant.userId.skills &&
                                      applicant.userId.skills.length > 0 && (
                                        <div className="flex flex-wrap gap-1 mt-1.5">
                                          ...
                                          {applicant.userId.skills.length >
                                            3 && (
                                            <span className="text-xs text-gray-400">
                                              +
                                              {applicant.userId.skills.length -
